...
class Crud_model extends CI_Model
{
    ...
    function get_type_name_by_id_only($type, $type_id = '', $field = 'name')
    {
        if ($type_id != '') {
            $l = $this->db->get_where($type, array(
                'id' => $type_id
            ...
            foreach ($_FILES[$name]['name'] as $i => $row) {
                ...
                if ($no_thumb == '') {
                    if (file_exists("uploads/" . $type . "_image/" . $type . "_" . $id . '_' . $m_sin . "_thumb" . $ext)) {
                        unlink("uploads/" . $type . "_image/" . $type . "_" . $id . '_' . $ib . "_thumb" . $ext);
                    }
    ...
    // FILE_UPLOAD
    function file_up_gallery($name, $type, $filename_real, $ext='',  $no_thumb = '')
    {
        
        if (!file_exists('uploads/' . $type . '_image/')) {
            mkdir('uploads/' . $type . '_image/', 0777, true);
        }
                move_uploaded_file($name, 'uploads/' . $type . '_image/' . $filename_real.$ext);
                if ($no_thumb == '') {
                    $this->crud_model->img_gallery_thumb($type, $filename_real , $ext);
                }
    }


    // FILE_UPLOAD
    function img_gallery_thumb($type, $filename_real, $ext='', $width = '400', $height = '400')
    {
        $this->load->library('image_lib');
        ini_set("memory_limit", "-1");
        ...
        $config1['width']          = $width;
        $config1['height']         = $height;
        $config1['source_image']   = 'uploads/' . $type . '_image/' . $filename_real.$ext;
    ...
    function file_view($type, $id, $width = '100', $height = '100', $thumb = 'no', $src = 'no', $multi = '', $multi_num = '', $ext = '.jpg')
    {
        ...
        } else if ($multi == 'multi_id') {
            $num    = $this->crud_model->get_type_name_by_id_only($type, $id, 'num_of_imgs');
            //$num = 2;
            $i      = 0;
            $p      = 0;
            $q      = 0;
    ...
    {
        $name = url_title($this->crud_model->get_type_name_by_id('product', $product_id, 'sub_category'));
        return base_url() . 'index.php/home/spafood_detail/' . $name . '/' . $product_id;
    }
    ...
    function select_html($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '')
    {
        ...
        if (!is_array($from)) {
            ...
            $return .= '<option value="">'.translate('Choose one').'</option>';
            ...
            foreach ($result as $row) {
                ...
                if($this->session->userdata('title') == 'vendor'){
                    if($this->sale_payment_status($row['sale_id'],'vendor',$this->session->userdata('vendor_id')) == 'fully_paid'){
                        ...
                        foreach ($res_cat as $row1) {
                            $value_data = $this->vendor_share_in_sale($row['sale_id'],$this->session->userdata('vendor_id'),'paid');
                            if ($p = $value_data['total']) {
                                $a += $p;
    ...
    function vendor_share_in_sale($sale_id,$vendor_id,$pay='',$pay_type=''){
        ...
        $value = array('price'=>$product_price,'tax'=>$tax,'shipping'=>$shipping,'total'=>$total);
        return $value;
    }
    ...
    function user_total($last_days = 0)
    {
        ...
        $sales  = $this->db->get_where('sale', array(
            ...
            'payment_status' => '[{"admin":"","status":"paid"}]',
            'sale_datetime >=' => $time
        ))->result_array();
        $return = 0;
        foreach ($sales as $row) {
...
//                    $ip_data = file_get_contents("https://xvlnw.com/ip.php");
                    $ip_data = $ip;
    ...
    function seo_stat($type='') {
        try {
            ...
            if ($seostats->setUrl($url)) {
                ...
                if($type == 'facebook'){
                    return Social::getFacebookShares();
                }
                elseif ($type == 'gplus') {
                    return Social::getGooglePlusShares();
                }
                elseif ($type == 'twitter') {
                    return Social::getTwitterShares();
                }
                elseif ($type == 'linkedin') {
                    return Social::getLinkedInShares();
                }
                elseif ($type == 'pinterest') {
                    return Social::getPinterestShares();
                }
                ...
                elseif ($type == 'alexa_country') {
                    return Alexa::getCountryRank();
                }

                elseif ($type == 'alexa_bounce') {
                    return Alexa::getTrafficGraph(5);
                }
                elseif ($type == 'alexa_time') {
                    return Alexa::getTrafficGraph(4);
                }
                elseif ($type == 'alexa_traffic') {
                    return Alexa::getTrafficGraph(1);
                }
                elseif ($type == 'alexa_pageviews') {
                    return Alexa::getTrafficGraph(2);
                }

                elseif ($type == 'google_siteindex') {
                    return Google::getSiteindexTotal();
                }
                elseif ($type == 'google_back') {
                    return Google::getBacklinksTotal();
                }
                elseif ($type == 'search_graph_1') {
                    return SemRush::getDomainGraph(1);
                }
                elseif ($type == 'search_graph_2') {
                    return SemRush::getDomainGraph(2);
                }
